...
def checkQLYQ(Content,wordLen,formatftbjList):
    ...
    def getRefList():
        ...
        logging.info("getRefList numList: %s", numList)           # 对应段落编号中引用的权利要求书
        logging.info("getRefList targetStrList: %s", targetStrList) 
        logging.info("getRefList paragList: %s", paragList)

        return paracodeList, numList, targetStrList, paragList

    def getRefJson():
        # global resJson
        paracodeList, numList, targetStrList, paragList = getRefList()
    ...
    def nameDiff():
        ...
        resJson = getRefJson()  # 获取权利要求之间的引用关系(eg:{'1': [], '2': [1], '3': [1, 2], '4': [1, 2], '5': [1, 2, 4], '6': [1, 2, 4, 5]})
    ...
    def refError():
        ...
        strValue = ""
        if len(paracodeList) == len(numList):
            strValue = '{'
            for i in range(len(paracodeList)):
                strValue = strValue + '"' + \
                    str(paracodeList[i]) + '"' + ':' + str(numList[i])
        ...
        return jsonValue, maxParaCode, targetStrList
    ...
    if jsonInfo != "":
        ...
        keyValue = getRefJson()
        logging.info("keyValue: %s", keyValue)
        ...
        for k, v in rowColumnAndWordDict.items():
            ...
            beqlyqNumList = keyValue[qlyqNum]      # beqlyqNumList : qlyqNum所引用的权利要求列表
