...
def main_bF():
    ...
    def auto_change_word(wordhandle, file_path):
    ...
    def batchFileReplace():
        ...
        if folderP != "":
            ...
            if folderPath.endswith('.docx') or folderPath.endswith('.doc'):
                strList, inList = auto_change_word(word, folderPath)  # strList为要标记到pdf上的字符串内容
                ...
                if len(inList) > 0:
                    endHint = endHint + "附图标记中存在有包含关系的名词，" + str(inList) + ",请注意一下！"
    ...
    lable2.grid(row=0, column=1,padx=(20,0))
    ...
    text2.grid(row=0, column=1,padx=(20,0))
    # 拖放Pdf文件
    ...
    frame3.pack()
    # 开始替换按钮
    bStartCheck = Button(root, width = 30, text='开始替换', command= lambda: batchFileReplace())
    bStartCheck.pack(pady=(15))
